/**
 * Tests derive from spec LTTS-02's ACs: POST `{voice, text}` to
 * `<LOCAL_TTS_URL>/speak` with `text` = "<title>. <message>" (or just the
 * message when there's no title); non-2xx/unreachable -> throws; registry
 * entry declares the required config keys.
 */
...
describe('LocalTtsChannel', () => {
  it('POSTs the configured player URL with the voice and combined title+message text', async () => {
    ...
    expect(http.calls).toEqual([
      {
        ...
        body: { voice: 'Luciana', text: 'Build finished. All tests passed' }
      }
    ])
  })

  it('sends just the message (no leading ". ") when the notification has no title', async () => {
    const http = new FakeHttpClient()
    const channel = new LocalTtsChannel(
