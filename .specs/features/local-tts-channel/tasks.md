...
**What**: New `src/admin/ui/admin-searchable-combobox.js` (pure filter/highlight/enter-selection helpers: `filterOptions`, `moveHighlightIndex`, `resolveEnterSelection` — DOM-free, unit-tested) + `src/admin/ui/admin-searchable-combobox-dom.js` (`createSearchableCombobox({container, options, initialValue, onSelect, inputId})`: text input + absolutely-positioned dropdown panel, live substring filtering across name/locale/sample, keyboard nav (ArrowUp/Down clamped not wrapped, Enter, Escape), click-outside-to-close via a document listener added on open/removed on close). Wired into `admin-local-tts.js` (`renderCombobox` replaces the old `renderSelect`; `buildVoiceOptions` now also emits a `searchText` field), same fallback-to-plain-text-input behavior when the player is unreachable (LTTS-03 AC2, unchanged). New dark-theme combobox rules added to `admin-instances.css`, replacing the now-dead `select.voice-select` rules. Unit tests: `admin-searchable-combobox.test.js` (20 cases: empty/whitespace query, substring match across all 3 fields, case-insensitivity, order preservation, no-mutation, highlight clamping, enter-selection resolution) + `admin-local-tts.test.js` updated for the new `searchText` field. DOM-wiring half (`admin-searchable-combobox-dom.js`) has no automated test (no jsdom/DOM-testing dependency in this repo, matching the project's existing convention of code-reviewing/live-verifying DOM-heavy render code) — verified by live Docker check instead (static files served, referenced correctly, container logs clean).
**Requirement**: LTTS-05 · **Tests**: unit (20 new, 407 total vs 387) + existing e2e/build gates · **Gate**: full
