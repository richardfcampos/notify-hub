...
- [ ] **D4-D8 Phase 2 (gateway rewire) — NEXT**: type-keyed registry + `buildInstance`; delivery reads instance from repo at delivery time (hot-reload); dispatch resolves instance ids ∩ enabled; API token→profile + /notify + /channels by instance id; container/entrypoints open DB + seed, remove startup fail-fast/buildActive. NOTE: existing api/worker still on old .env model — Phase 2 rewires them; keep tests green.
